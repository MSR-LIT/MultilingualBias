...
from settings import CACHE_DIR, EMBEDDINGS_FILENAME
from config import parse_args, TrainConfig
from utils import create_data_loader, to_device, get_trainable_parameters, save_weights, load_pickle, init_weights
from preprocess import load_bios_counts, load_bios_seq, load_titles, load_features_names, load_vocab
...
def create_embeddings(word_embeddings, vocab):
    embeddings_size = len(next(iter(word_embeddings.values())))
...
def create_dataset(config):
    ...
    def add_dataset_param(name, value_train, value_dev=None, value_test=None):
        ...
        dataset_params_val[name] = value_dev if value_dev is not None else value_train
        dataset_params_test[name] = value_test if value_test is not None else value_train
    ...
    print(f'Dataset: {type(dataset_train).__name__} - {len(dataset_train)}, {len(dataset_val)}, {len(dataset_test)}')
...
def main(cfg):
    ...
    data_loader_train = create_data_loader(dataset_train, cfg.batch_size, shuffle=True, num_workers=3)
    data_loader_dev = create_data_loader(dataset_val, cfg.batch_size, shuffle=False, num_workers=3)
    print(f'Data loader: {len(data_loader_train)}, {len(data_loader_dev)}')

    # weighted loss
    if cfg.use_class_weight:
        classes_weights = compute_class_weight('balanced', np.unique(dataset_train.titles), dataset_train.titles)
    ...
    def eval_model(engine):
        ...
        if best_val_loss > metrics_dev['loss'] or engine.state.epoch <= cfg.min_epochs:
            best_val_loss = metrics_dev['loss']
            nb_epoch_no_improvement = 0

            save_weights(model, os.path.join(CACHE_DIR, f'model_{cfg.model}.pt'))
