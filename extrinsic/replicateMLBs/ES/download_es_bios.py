...
with open("freq_titles_es.json", "r") as f:
...
bad_regex = re.compile(r"\b(?:yo|tú|usted|tu|mí|me|conmigo|mi|mía|mío|nuestro|nuestra|nuestros|nuestras|nosotro|nosotros|nosotra|nosotras)\b", flags=re.I)
m_regex = re.compile(r"\b(?:el sr|es un|el señor|el suyo|el suyos|él|él mismo|sí mismo)\b", flags=re.I)
f_regex = re.compile(r"\b(?:la sra|es una|la señora|la suya|las suyas|ella|ella misma|sí misma)\b", flags=re.I)
...
if __name__ == "__main__":
    if not args.wetpaths.endswith("wet.paths"):
        ...
        path_root = "es-CC-MAIN-{}-".format(args.wetpaths)
    else:
