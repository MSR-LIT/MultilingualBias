...
def process(p, replacement="_"):
    ...
    regExp = r"\b(?:[Ee]r|[Ss]ie|[Ii]hr|[Ss]eine|[Ii]hm|[Ii]hres|[Ss]elbst|[Ss]ie selber|[Hh]err|[Ff]rau|[Mm][Rr]|[Mm][Rr][sS]|[Mm][Ss]|"
...
def dedup_middle(bios): # remove triples where the middle name is a prefix of another middle name, so {Mary Lynn Doe, Mary L Doe, Mary Doe} => {Mary Lynn Doe}, but {Mary L Doe, Mary I Doe} => {Mary L Doe, Mary I Doe}
    trips = group_by(bios, lambda b: (b["title"], b["name"][0], b["name"][-1]))
    for k in trips:
        to_remove = set()
        if len(k)==1:
            continue
...
def dedup(people): 
    ...
    print(f"all examples:{len(ans)}")
    for x in ans:
        if x["title"] == 'journalist':
            print(x['title'] + ';\t' + ' '.join(x["name"]) + '\n')
    return ans
