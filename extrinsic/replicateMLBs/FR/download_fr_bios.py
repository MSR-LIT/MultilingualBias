...
esnlp = spacy.load('fr')
esnlp.add_pipe(LanguageDetector(), name='language_detector', last=True)
...
with open("freq_titles_fr.json", "r") as f:
    freq_titles = json.load(f)

lower_freq_titles = {t.lower(): normalized.lower() for t, normalized in freq_titles.items()}
...
# print(freq_titles_regex)

bad_regex = re.compile(r"\b(?:je|toi|votre|moi|mon|mienne|mien|moi même|notre|les notres|la nôtre|les nôtres|nous|nous-mêmes)\b", flags=re.I)
m_regex = re.compile(r"\b(?:Monsieur|il|celui|lui-même|sa personne)\b", flags=re.I)
f_regex = re.compile(r"\b(?:Mme|elle|celle|elle-même)\b", flags=re.I)
sentence_end_regex = re.compile(r"\. +[A-ZÉ]")
...
def spacy_extract_name(name_strings):
    doc = esnlp(name_strings)
    names = [str(x) for x in doc.ents if x.label_ == 'PERSON' or x.label_ == 'PER']
    if len(names) != 1:
        return None
    tokens = names[0].strip().split(' ')
    if len(tokens) == 1:
        return (tokens[0].strip().replace(".", ""), '', '')
    elif len(tokens) == 2:
        return (tokens[0].strip().replace(".", ""), "", tokens[1].strip().replace(".", ""))
    return tuple([x.strip().replace(".", "") for x in tokens])
...
def extract_bios_from_page(page, URI, max_preceed=MAX_PRECEED, min_len=MIN_LENGTH):
    ...
    #     return [] # maybe not ES
    if " est " not in page:
        return []  # probably not FR
    ISA = " est " # in FR, when talking about occupation, indefinite article will be ignored
    # ISAN = " es una "
    ...
    for line_str in page.split('\n'):
        if len(line_str) >= min_len:
            ...
            m = re.match(freq_titles_regex, line_str[b:]) # is an architect; has one occupation word after b-th position
...
#                     line_str = line_str[:weird]
                ...
                if not g or line_str[end:].startswith(",") or line_str[end:].startswith(" y "): # avoid 'is an architect and' or 'is an architect, blah' 
                ...
                name = spacy_extract_name(line_str[:a])
                if not name:
                    continue
                matches.append(
...
def bios_from_wet_url(url, verbose=False):
    try:
        ...
        # for x in it:
        #     print(x.rec_headers, x.format, x.rec_type, x.raw_stream, x.http_headers, x.content_type, x.length)
        #     break
        # sys.exit()

        ans = dedup_exact([bio for record in it for bio in
               extract_bios_from_page(record.content_stream().read().decode()[:MAX_PAGE_LEN], \
...
if __name__ == "__main__":
    if not args.wetpaths.endswith("wet.paths"):
        ...
        path_root = "fr-CC-MAIN-{}-".format(args.wetpaths)
    else:
