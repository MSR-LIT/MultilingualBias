...
with open("freq_titles_de.json", "r") as f:
...
bad_regex = re.compile(r"\b(?:Ich|du|dein|deine|dein|euer|eure|euer|mich|mir|meine|mein|meiner|meines|unsere|unser|unseres|uns|wir)\b", flags=re.I)
bad_regex2 = re.compile(r"\b(?:[a-zA-Z0-9_,]+ Sie|[a-zA-Z0-9_,]+ Ihre)\b")
m_regex = re.compile(r"\b(?:er|seine|seiner|seines|ihn|ihm|Herr)\b", flags=re.I)
f_regex = re.compile(r"\b(?:sie|ihre|ihres|sie selbst|Frau|Fräulein)\b", flags=re.I)
sentence_end_regex = re.compile(r"\. +[A-ZÉ]")

def infer_gender(bio):
    if re.search(bad_regex, bio) or re.search(bad_regex2, bio):
        return None
...
if __name__ == "__main__":
    if not args.wetpaths.endswith("wet.paths"):
        ...
        path_root = "de-CC-F-MAIN-{}-".format(args.wetpaths)
    else:
